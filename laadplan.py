...
while idx < len(laad_lijst):
    ...
    if heeft_buur:
        ...
        vulling_kleur2 = "#e74c3c" if (start_x + item2["L"] > max_lengte) else item2["kleur"]
        rect2 = patches.Rectangle((start_x, max_breedte - item2["B"] - 20), item2["L"], item2["B"], linewidth=1, edgecolor='white', facecolor=vulling_kleur2, alpha=0.8)
        ax.add_patch(rect2)
        ...
        max_r = max(x_onder, x_boven); x_onder, x_boven = max_r, max_r
        idx += 2
    else:
        x_onder, x_boven = start_x + item["L"], start_x; idx += 1
