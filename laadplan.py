...
with col4:
    ...
    if pallets_cp9 > 0 and "CP9" not in st.session_state.klik_volgorde:
        st.session_state.klik_volgorde.append("CP9")
...
with col5:
    ...
    if pallets_mw > 0:
        if "Maatwerk" not in st.session_state.klik_volgorde:
            st.session_state.klik_volgorde.append("Maatwerk")
...
if pallets_cp3 + pallets_cp7_totaal + pallets_ibc + pallets_cp9 + pallets_mw > 0:
    with st.expander("🚚 Klik hier voor extra aslast-regelingen (Midden vooraan/achteraan)"):
...
while idx < len(laad_lijst):
    item = laad_lijst[idx]
    dynamic_fs = max(8, min(14, int(item["L"] / 90)))
